...
Two local sources, both 5.2. Prefer whichever answers the question faster. The Blender MCP server's docs tools are a convenience, not the authority — confirm their bundled version first (see **Current Architecture State**):
...
- `Source/lib/spline_export_core.py` — shared logic. No UI, no operators, **no presentation**. `build_plan(context, settings)` is read-only and raises `PlanError`; `execute_plan(context, plan)` is the only mutating function. `ExportSettings` is the front-end-agnostic input shape, and is **frozen**. Verified end to end 2026-09-22, FBX write and Unreal import included. Hardening added and tested in Blender 2026-09-23 (see the module's STATUS).
  ...
  - **Depends on `mesh_simplify_core`** (imported as `simplify_core`, and re-exposed so front-ends build `SimplifySettings` from the same module object). It `importlib.reload`s that module at import time, because front-ends reload only what they import — without it, simplify edits stay invisible to the export until restart.
  - **Optional simplify step (added and tested 2026-09-23).** `ExportSettings.simplify` is `SimplifySettings | None`: one value carries on/off and parameters. `build_plan` analyses the *evaluated* mesh via `analyze_mesh()` in the same `to_mesh()` pass as the counts; `execute_plan` runs `simplify_mesh()` on the converted duplicate **right after convert, before transform_apply and origin_set** — same local space as the plan's analysis, and MEDIAN origin must average the final vertex set. Best-effort: not possible → non-blocking `SIMPLIFY_SKIPPED` warning and an unsimplified export. A plan/execute mismatch raises inside the rollback.
  - **Optional UV layout step (added and tested in Blender 2026-09-24).** `ExportSettings.uv_layout` is `UVSettings | None`; `uv_core` is imported and reloaded alongside `simplify_core`. `evaluate_source()` now returns a `SourceEvaluation` and owns the simplify go/no-go, because the UV prediction must know whether it is looking at a simplified mesh: it simplifies an in-memory bmesh copy (`simplify_core.simplify_bmesh`) and analyses that (`uv_core.analyze_bmesh`). `execute_plan` runs `layout_mesh()` **after transform_apply** (world-space proportions) and before origin_set; the re-check is topological, so the local-space plan still matches. **Not best-effort, unlike simplify:** an impossible layout is a blocking `UV_FAILED` warning, since the shader cannot use a mesh without it.
...
- `Source/ui/spline_to_unreal_panel.py` — panel front-end, registering **three** panels in the "Pinball" tab of the 3D Viewport sidebar: "Spline to Unreal", "Simplify", and "UV Layout" (added and tested 2026-09-24). Each uses two operators (`pinball.spline_preview`/`spline_export`, `pinball.simplify_preview`/`simplify_apply`, `pinball.uv_preview`/`uv_apply`) rather than a dry-run checkbox: a persisted "don't actually do it" toggle is invisible at click time and decays. Both apply-operators' `poll()` refuses unless a matching, non-blocked, non-stale preview exists — where "stale" means only that the active object's name changed; editing inputs after Preview does not invalidate it (considered and declined 2026-09-23). Registered and exercised 2026-09-22.
  - The export panel's **Simplify** and **UV Layout** checkboxes are only switches; the parameters are the Simplify and UV Layout panels' `PINBALL_PG_simplify` / `PINBALL_PG_uv_layout`, drawn under each checkbox by the shared `_draw_simplify_settings()` / `_draw_uv_settings()`. One set of settings each, visible in both places.
...
- `Source/lib/mesh_simplify_core.py` — dissolves redundant profile columns from a curve-converted mesh. `build_simplify_plan` / `execute_simplify_plan`, same contract as the export core. Executed 2026-09-22 on `InnerRearRamp` (2134 → 804 verts). End-cap rim fix and multi-user/library-linked refusal added and tested in Blender 2026-09-23.
  ...
  - **Two entry-point pairs over one engine.** Active-object: `build_simplify_plan` / `execute_simplify_plan` (Edit or Object mode, ownership checks). Mesh-level, for callers that own the mesh: `analyze_mesh` / `simplify_mesh`. `SimplifyAnalysis` is plain values only, so it outlives a temporary evaluated mesh; `SimplifyPlan` subclasses it (both `kw_only`) adding `source` and `mesh`. Both mutating paths go through `_apply()`, which re-analyses and refuses unless width, rows, shells and the dissolve set all still match.
...
- `Source/lib/mesh_uv_core.py` — straight UV islands and seams for a converted, solidified ramp. `build_uv_plan` / `execute_uv_plan` (active object) and `analyze_bmesh` / `analyze_mesh` / `layout_mesh` (mesh-level, used by the export), same contract as the other cores. Written 2026-09-24 and tested in Blender the same day, both through the UV Layout panel and inside Convert & Export.
  - Three islands: **inner** (the shell with the shorter profile), **outer** (V **reversed** relative to inner — the shader depends on it), and **band** (every rim face: both long rims and both end caps, unrolled from one closed ring and cut once at row 0 / column 0). The band runs the same way as inner along column 0. Each island is scaled uniformly to fill 95% of the UV height, side by side — a single shared scale was rejected 2026-09-24 because the band, twice a shell's length, left the shells too small. Density therefore differs between islands, never within one; U is compressed uniformly only if the set would not fit (`width_fit`).
  - Positions come from the grid, not an unwrap: U and V are cumulative distances averaged over the island, so rows and columns are exactly straight. V is fixed by intent; U is then reversed per island if needed so no face is mirrored. The report measures mirrored faces per island — expect 0.
  - Reuses `mesh_simplify_core.recover_grid()`, so it inherits its refusals, including width < 3.
  - **Measured 2026-09-24: Simplify's output is still row-major.** `InnerRearRamp.001` after Simplify: width 4 × 97 rows × 2 shells, stride 388, 100% of 1548 edges explained; 774 faces = 576 shell + 198 rim. Meshes modelled before these scripts are not (`CentrifugeRamp.001`, data `Cube.033`: 61.7%) and are refused rather than walked — regenerate them from their curve.
- `Source/cli/uv_layout_mesh.py` — script front-end for the UV core, with a `DRY_RUN` flag.
...
- **Unverified assumptions (as of 2026-09-23).** Each is flagged in code; check it against a measurement the first time the case comes up.
  - Simplify on a **single-shell** mesh (no Solidify): the vertex prediction assumes open-boundary end-row vertices dissolve at 2 edges. Only 2-shell meshes have been run.
  - `width = 2 × bevel_resolution + 3` for PROFILE bevels is measured on **one** 5-point STEPS profile. `analyze_curve_bevel.py` prints MATCH/MISMATCH against the measured width, so another profile will show it.
  - The ROUND-bevel width formula `4 × (bevel_resolution + 1)` has never been measured on this project.
  - UV layout on a **single-shell** mesh (no Solidify, so no band): supported in code, one island, never run.
...
- The Blender MCP server is connected. The `mcp` add-on (Blender Lab, v1.0.3) listens on localhost:9876 inside Blender; the MCP server that Claude talks to is a separate process. Connection verified live against `D:\dev\blender_models\pinball.blend`.
  - **How Code sessions get it (set up 2026-09-24).** Claude Desktop's "Blender" extension serves only Desktop's Chat; Code sessions do not see it. The server is installed from a clone of `https://projects.blender.org/lab/blender_mcp.git` at `D:\dev\Blender\blender_mcp` — the package is in its `mcp\` subfolder, so a `pip install git+<repo root>` fails — editable-installed into its own venv `D:\dev\Blender\blender_mcp_venv`, and registered local-scope for this repo as `Blender` → `D:\dev\Blender\blender_mcp_venv\Scripts\blender-mcp.exe`. Update with `git pull` in the clone.
  - **Name trap:** `pip install blender-mcp` from PyPI is a *different* project (ahujasid's, now `mcp-for-blender`) with its own add-on. `blender-mcp-connect` on PyPI is an unofficial downstream. Neither is the Blender Lab server.
  - Tool surface: scene inspection (`get_objects_summary`, `get_object_detail_summary`, `get_blendfile_summary_*`), docs search (`search_api_docs`, `search_manual_docs`, `get_python_api_docs`), screenshots and renders (`get_screenshot_of_*`, `render_viewport_to_path`, `render_thumbnail_to_path`), UI navigation (`jump_to_*`), and `execute_blender_code`. Several tools also have a `*_for_cli` variant.
  - **Script Execution Responsibility and Ask-before-any-mutation both govern.** `execute_blender_code` and `execute_blender_code_for_cli` are off-limits outright — it is the exact thing Script Execution Responsibility forbids, and the MCP server's own instructions telling Claude to prefer operators and drive the scene do not override the user's rules. The mutating tools — `jump_to_*` (moves the user's UI out from under them) and `render_*_to_path` (writes files, can be expensive) — require asking first, every time.
  ...
  - **MCP docs version.** `search_api_docs`, `get_python_api_docs`, and `search_manual_docs` read RST bundled with the server, now `D:\dev\Blender\blender_mcp\mcp\blmcp\data`, whose `api/index.rst` self-identifies as **Blender 5.2** (as of the 2026-09-24 clone). The Desktop extension's copy was 5.1 — 85 `bpy.types` pages short of 5.2 — which is why these tools were previously banned for verification. Re-check `index.rst` after any `git pull`; the local 5.2 sources under **Blender API Verification** remain the authority.
  - Claude Desktop is a packaged (MSIX) app, so Windows redirects its `AppData\Roaming` writes to `C:\Users\jhouk\AppData\Local\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Roaming\`. Processes Desktop spawns see those files at `AppData\Roaming\Claude\...`; Explorer and ordinary terminals do not, so give the user the physical path.
