...
- `Source/ui/spline_to_unreal_panel.py` — panel front-end. 3D Viewport sidebar, "Pinball" tab. Two operators (`pinball.spline_preview`, `pinball.spline_export`) rather than a dry-run checkbox: a persisted "don't actually do it" toggle is invisible at click time and decays. Export's `poll()` refuses unless a matching, non-blocked preview exists. First draft, never run.
